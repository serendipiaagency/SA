  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.heroStrip}>
        {heroImg && <img src={heroImg} alt={dest.name} style={s.heroBg} onError={e => { e.target.style.display = 'none'; }} />}
        ...
        <div style={s.heroContent}>
          <div style={s.pills}>
            ...
            <span style={{ ...s.pill, background: '#F59E0B', color: '#1A1A1A', fontWeight: 700 }}>💶 €{budgetUsed.toFixed(0)} total</span>
          </div>
          <h1 style={s.title}>{dest.name ? `Tu itinerario en ${dest.name}` : 'Tu itinerario está listo'}</h1>
      ...
      <div style={s.body}>
        <div style={s.dashboard}>
          ...
          <div style={s.totalSection}>
            ...
            <span style={s.totalSub}>Disponible: <strong>€{Math.max(0, itinerary.budget - budgetUsed).toFixed(0)}</strong></span>
          </div>
          <div style={s.dashCols}>
            <div>
              ...
              {Object.entries(catTotals).sort((a, b) => b[1] - a[1]).map(([cat, cost]) => (
                <div key={cat} style={s.miniRow}>
                  ...
                  <div style={s.miniTrack}><div style={{ ...s.miniFill, width: `${(cost / maxCat) * 100}%`, background: CAT_FG[cat] }} /></div>
            ...
            <div>
              ...
              {dayTotals.map(d => (
                <div key={d.day_number} style={s.miniRow}>
                  ...
                  <div style={s.miniTrack}><div style={{ ...s.miniFill, width: `${(d.cost / maxDay) * 100}%`, background: '#276749' }} /></div>
          ...
          return (
            <div key={day.day_number} style={s.dayBlock}>
              <div style={s.dayHeader}>
                ...
                <span style={s.dayCount}>{day.items.length} actividad{day.items.length !== 1 ? 'es' : ''}</span>
                {day.items.length > 0 && (
                  <button style={{ ...s.viewToggle, ...(showMap ? s.viewToggleOn : {}) }} onClick={() => toggleMap(day.day_number)}>
              ...
              {day.items.length === 0 ? (
                ...
                day.items.map((item, idx) => (
                  <ActivityCard key={item.activity.id} item={item} isFirst={idx === 0} prevActivity={idx > 0 ? day.items[idx - 1].activity : null} />
                ))
              )}
            </div>
          );
        ...
        <div style={s.cta}>
          <button style={s.shareCtaBtn} onClick={() => shareItinerary(itinerary.id)}>🔗 Compartir itinerario</button>
...
const s = {
  ...
  nav: { position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 24px', background: 'rgba(255,251,245,0.92)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(0,0,0,0.06)' },
  back: { background: 'none', border: '1.5px solid #E0DDD8', borderRadius: 8, padding: '7px 14px', fontSize: 13, fontWeight: 600, color: '#555' },
  logo: { fontSize: 20, fontWeight: 900, color: '#0D3B2E' },
  shareBtn: { background: '#F59E0B', border: 'none', borderRadius: 8, padding: '7px 14px', fontSize: 13, fontWeight: 700, color: '#1A1A1A' },
  heroStrip: { background: 'linear-gradient(135deg, #0D3B2E 0%, #276749 100%)', padding: '100px 24px 56px', position: 'relative', overflow: 'hidden', minHeight: 300 },
  heroBg: { position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', zIndex: 0 },
  heroOverlay: { position: 'absolute', inset: 0, zIndex: 1, background: 'linear-gradient(135deg, rgba(13,59,46,0.82) 0%, rgba(39,103,73,0.72) 100%)' },
  heroContent: { maxWidth: 760, margin: '0 auto', position: 'relative', zIndex: 2 },
  pills: { display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 20 },
  pill: { background: 'rgba(255,255,255,0.15)', color: '#fff', fontSize: 13, fontWeight: 600, padding: '6px 14px', borderRadius: 20, border: '1px solid rgba(255,255,255,0.2)' },
  ...
  dashboard: { background: '#fff', borderRadius: 20, padding: '28px 32px', boxShadow: '0 4px 24px rgba(0,0,0,0.07)', marginBottom: 48, border: '1.5px solid #F0EDE8' },
  ...
  dayHeader: { display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20, paddingBottom: 14, borderBottom: '2px solid #F0EDE8', flexWrap: 'wrap' },
  ...
  dayCost: { fontSize: 12, fontWeight: 700, color: '#92400E', background: '#FEF3C7', border: '1px solid #FCD34D', padding: '3px 10px', borderRadius: 20 },
  dayCount: { fontSize: 12, color: '#999', marginLeft: 'auto' },
  viewToggle: { fontSize: 12, fontWeight: 700, padding: '5px 12px', borderRadius: 20, border: '1.5px solid #E0DDD8', background: '#FAFAF8', color: '#555' },
  viewToggleOn: { background: '#0D3B2E', color: '#fff', borderColor: '#0D3B2E' },
  empty: { color: '#AAA', fontStyle: 'italic', padding: '16px 0' },
  cta: { textAlign: 'center', padding: '48px 0 0', borderTop: '1px solid #EEE', display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' },
  shareCtaBtn: { background: '#F59E0B', color: '#1A1A1A', border: 'none', padding: '14px 28px', borderRadius: 12, fontSize: 15, fontWeight: 700 },
  ctaBtn: { background: '#0D3B2E', color: '#fff', border: 'none', padding: '14px 28px', borderRadius: 12, fontSize: 15, fontWeight: 700 },
};

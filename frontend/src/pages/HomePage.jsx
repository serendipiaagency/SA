      ...
      <section id="destinos" style={s.section}>
        ...
        <div style={s.grid}>
          {destinations.map((dest, i) => {
            ...
            const photoUrl = photoId ? `https://images.unsplash.com/photo-${photoId}?w=700&auto=format&fit=crop&q=75` : null;
            return (
              <button key={dest.id} style={{ ...s.card, animationDelay:`${i * 0.05}s` }} className="fade-up" onClick={() => onPickDest(dest)}>
                <div style={{ ...s.cardBg, background: theme.gradient }}>
                  {photoUrl && <img src={photoUrl} alt={dest.name} style={s.cardPhoto} onError={e => { e.target.style.display = 'none'; }} />}
...
const s = {
  ...
  nav: { position:'fixed', top:0, left:0, right:0, zIndex:100, display:'flex', alignItems:'center', gap:12, padding:'16px 32px', background:'rgba(255,251,245,0.88)', backdropFilter:'blur(12px)', borderBottom:'1px solid rgba(0,0,0,0.06)' },
  logo: { fontSize:22, fontWeight:900, letterSpacing:-1, color:'#0D3B2E' },
  navTag: { fontSize:13, color:'#999' },
  hero: { minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'linear-gradient(160deg,#0D3B2E 0%,#1B5E42 50%,#276749 100%)', position:'relative', overflow:'hidden', paddingTop:80 },
  ...
  card: { background:'#fff', borderRadius:20, overflow:'hidden', border:'none', textAlign:'left', padding:0, boxShadow:'0 4px 24px rgba(0,0,0,0.08)', transition:'transform 0.2s,box-shadow 0.2s', cursor:'pointer' },
